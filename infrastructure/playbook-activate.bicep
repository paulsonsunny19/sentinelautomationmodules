...
@minLength(5) param namePrefix string
...
@allowed(['upn','ip','cidr','fqdn']) param watchlistKeyDataType string = 'upn'
...
resource sentinelConnection 'Microsoft.Web/connections@2016-06-01'={name:sentinelConnectionName location:location kind:'V1' properties:{displayName:sentinelConnectionName customParameterValues:{} parameterValueType:'Alternative' api:{id:sentinelManagedApiId}}}
resource activatedPlaybook 'Microsoft.Logic/workflows@2019-05-01'={
 name:playbookName location:location identity:{type:'SystemAssigned'} properties:{state:'Enabled' definition:{'$schema':'https://schema.management.azure.com/providers/Microsoft.Logic/schemas/2016-06-01/workflowdefinition.json#' contentVersion:'2.1.0.0' parameters:{'$connections':{type:'Object' defaultValue:{}} ProjectName:{type:'String' defaultValue:'STATNext'} PlaybookVersion:{type:'String' defaultValue:'2.1.0-full'}} triggers:{When_Microsoft_Sentinel_incident_was_created:{type:'ApiConnectionWebhook' inputs:{body:{callback_url:'@{listCallbackUrl()}'} host:{connection:{name:'@parameters(\'$connections\')[\'azuresentinel\'][\'connectionId\']'}} path:'/incident-creation'}}} actions:{
 ...
 AAD_Risks:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_aad_risks' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' lookbackDays:14} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 Related_Alerts:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_related_alerts' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' lookbackDays:relatedAlertsLookbackDays alertKqlFilter:relatedAlertsKqlFilter checkAccounts:true checkHosts:true checkIPs:true} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 Threat_Intelligence:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_threat_intel' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' lookbackDays:14} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 MDE:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_mde' body:{base:'@body(\'Build_STAT_Base\')' lookbackDays:14} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 UEBA:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_ueba' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' lookbackDays:14 minimumInvestigationPriority:1} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 File_Insights:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_file' body:{base:'@body(\'Build_STAT_Base\')'} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 Defender_for_Cloud_Apps:{type:'Http' runAfter:{Build_STAT_Base:['Succeeded']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_mcas' body:{base:'@body(\'Build_STAT_Base\')' scoreThreshold:mcasScoreThreshold} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 Optional_KQL:{type:'If' expression:'@not(empty(\'${replace(enrichmentKql, '\'', '\'\'')}\'))' runAfter:{Build_STAT_Base:['Succeeded']} actions:{Run_KQL:{type:'Http' inputs:{method:'POST' uri:'${functionBaseUri}/stat_kql' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' query:enrichmentKql lookbackDays:14} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}} else:{actions:{KQL_Skipped:{type:'Compose' inputs:{ModuleName:'KQLModule' ResultsCount:0 ResultsFound:false}}}}}
 Optional_Watchlist:{type:'If' expression:'@not(empty(\'${replace(watchlistAlias, '\'', '\'\'')}\'))' runAfter:{Build_STAT_Base:['Succeeded']} actions:{Run_Watchlist:{type:'Http' inputs:{method:'POST' uri:'${functionBaseUri}/stat_watchlist' body:{workspaceId:sentinelWorkspaceId base:'@body(\'Build_STAT_Base\')' watchlistAlias:watchlistAlias watchlistKey:watchlistKey watchlistKeyDataType:watchlistKeyDataType} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}} else:{actions:{Watchlist_Skipped:{type:'Compose' inputs:{ModuleName:'WatchlistModule' WatchlistMatchCount:0}}}}}
 Score_STAT:{type:'Http' runAfter:{AAD_Risks:['Succeeded','Failed'] Related_Alerts:['Succeeded','Failed'] Threat_Intelligence:['Succeeded','Failed'] MDE:['Succeeded','Failed'] UEBA:['Succeeded','Failed'] File_Insights:['Succeeded','Failed'] Defender_for_Cloud_Apps:['Succeeded','Failed'] Optional_KQL:['Succeeded','Failed'] Optional_Watchlist:['Succeeded','Failed']} inputs:{method:'POST' uri:'${functionBaseUri}/stat_scoring' body:{inputs:[{module:'@body(\'AAD_Risks\')'},{module:'@body(\'Related_Alerts\')'},{module:'@body(\'Threat_Intelligence\')'},{module:'@body(\'MDE\')'},{module:'@body(\'UEBA\')'},{module:'@body(\'File_Insights\')'},{module:'@body(\'Defender_for_Cloud_Apps\')'}]} authentication:{type:'ManagedServiceIdentity' audience:'https://management.azure.com/'}}}
 ...
 } outputs:{}} parameters:{'$connections':{value:{azuresentinel:{connectionId:sentinelConnection.id connectionName:sentinelConnection.name id:sentinelManagedApiId connectionProperties:{authentication:{type:'ManagedServiceIdentity'}}}}}}} dependsOn:[sentinelConnection,bootstrapPlaybook]}
