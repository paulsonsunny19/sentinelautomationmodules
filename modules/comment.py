...
def build_comment(base,scoring,aad=None,related=None,ti=None,ip_baseline=None,mde=None,ueba=None,file_insights=None,mcas=None,oof=None):
    aad,related,ti,ip_baseline,mde,ueba,file_insights,mcas,oof=[x if isinstance(x,dict) else {} for x in (aad,related,ti,ip_baseline,mde,ueba,file_insights,mcas,oof)]
    score=scoring.get("TotalScore",0);degraded=bool(base.get('EnrichmentWarnings') or aad.get('RiskUserAvailable') is False or aad.get('RiskEventsAvailable') is False or related.get('EnrichmentWarnings') or ip_baseline.get('EnrichmentWarnings') or ueba.get('EnrichmentWarnings') or oof.get('EnrichmentWarnings'))
    ...
    if ip_rows:
        ip_section="<h3>IP Info</h3>"+_html_table(["IP","City","State","Country","Organization","OrganizationType","ASN"],ip_rows)
        if base.get('EnrichmentWarnings'):ip_section+="<p><strong>IP enrichment warning:</strong> "+_html(_warnings(base))+"</p>"
    ...
    for item in oof.get("DetailedResults",[]):
        if isinstance(item,dict):oof_rows.append([_mailto(item.get("UPN")),item.get("OOFStatus"),item.get("InternalMessage"),item.get("ExternalMessage")])
    if oof_rows:sections.append("<h3>Automatic Replies</h3>"+_html_table(["User","OOF Status","Internal Reply","External Reply"],oof_rows))
    module_rows=[["AAD / Identity Risk",f"User risk: {aad.get('HighestRiskLevel','unknown')}; risk events: {aad.get('RiskEventCount',0) if aad.get('RiskEventsAvailable',True) else 'Unavailable'}; failed MFA: {aad.get('FailedMFATotalCount',0)}; fraud: {aad.get('MFAFraudTotalCount',0)}",_warnings(aad)],["Related Alerts",related.get("RelatedAlertsCount",0),_context(related,[("RelatedAlertsCount","Related alerts")])],["Threat Intelligence",ti.get("MatchedTIItemCount",0),_context(ti,[("MatchedTIItemCount","TI matches")])],["IP GeoData",base.get("GeoEnrichedIPsCount",0),_context(base,[("PublicIPsCount","Public IPs"),("GeoEnrichedIPsCount","Geo enriched")])],["IP Network Baseline",ip_baseline.get("IPsObservedCount",0),_context(ip_baseline,[("IPsAnalyzedCount","IPs analyzed"),("IPsObservedCount","IPs observed"),("IsolatedNewPeerCount","Isolated peers"),("EstablishedEstatePeerCount","Established peers")])],["MDE",mde.get("AnalyzedEntities",mde.get("MachineCount",0)),_context(mde,[("AnalyzedEntities","Entities"),("MachineCount","Machines")])],["UEBA",ueba.get("AnomalyCount",0),_context(ueba,[("AnomalyCount","Anomalies")])],["File Insights",file_insights.get("HashesLinkedToThreatCount",0),_context(file_insights,[("HashesLinkedToThreatCount","Threat-linked hashes")])]]
    ...
    if oof:module_rows.append(["Automatic Replies",f"Out of office: {oof.get('UsersOutOfOffice',0)}; In office: {oof.get('UsersInOffice',0)}; Unknown: {oof.get('UsersUnknown',0)}",_context(oof,[("UsersAnalyzed","Users analyzed")])])
