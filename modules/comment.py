...
def _html_table(headers: list[str], rows: list[list[Any]]) -> str:
    ...
    for row in rows:
        cells=[f"<td>{v}</td>" if isinstance(v,_SafeHtml) else f"<td>{_html(v)}</td>" for v in row]
...
def build_comment(base: dict[str,Any],scoring: dict[str,Any],aad: dict[str,Any]|None=None,related: dict[str,Any]|None=None,ti: dict[str,Any]|None=None,mde: dict[str,Any]|None=None,ueba: dict[str,Any]|None=None,file_insights: dict[str,Any]|None=None,mcas: dict[str,Any]|None=None) -> dict[str,Any]:
    ...
    details=[x for x in aad.get("DetailedResults",[]) if isinstance(x,dict)]; risk_by_upn={str(x.get("UserPrincipalName","")).lower():x for x in details}; risk_by_id={str(x.get("UserId","")).lower():x for x in details if x.get("UserId")}
    ...
    for item in base.get("IPs",[]):
        r=_raw(item); geo=item.get("GeoData") if isinstance(item.get("GeoData"),dict) else {}
        ip_rows.append([item.get("Address") or r.get("address"),geo.get("city"),geo.get("state"),geo.get("country"),geo.get("organization"),geo.get("organizationType"),geo.get("asn")])
    if ip_rows: sections.append("<h3>IP Info</h3>"+_html_table(["IP","City","State","Country","Organization","OrganizationType","ASN"],ip_rows))

    events=[x for x in aad.get("RiskEvents",[]) if isinstance(x,dict)]; aad_warnings=[str(x) for x in aad.get("EnrichmentWarnings",[])]; identity_failure=any(x.startswith(('Risk detections:','Risky user lookup:')) for x in aad_warnings)
    if events:
        rows=[[_mailto(e.get("UserPrincipalName")),e.get("RiskEventType"),e.get("RiskLevel"),e.get("RiskState"),e.get("RiskDetail"),e.get("Activity"),e.get("IPAddress"),e.get("DetectedDateTime")] for e in events[:20]]
        ...
        failures='; '.join(x for x in aad_warnings if x.startswith(('Risk detections:','Risky user lookup:')))
        sections.append("<h3>Entra ID Protection - Risky Events</h3><p><strong>Enrichment unavailable.</strong> "+_html(failures)+"</p>")
