...
def normalize_entities(
    ...
    for entity in entities:
        ...
        if kind == 'account':
            accounts.append({
                'UserPrincipalName': p.get('userPrincipalName') or p.get('upn') or p.get('friendlyName'),
            ...
            parsed = _parsed_ip(address)
            ips.append({
                'Address': str(parsed) if parsed else address,
                'IsPublic': bool(parsed and parsed.is_global),
                'RawEntity': raw,
            })
        elif kind == 'host':
    ...
    return {
        ...
        'IPs': ips,
        'IPsCount': len(ips),
        'PublicIPsCount': sum(1 for item in ips if item.get('IsPublic')),
        'Hosts': hosts,
...
def normalize(
    ...
    if tenant_display_name:
        result['TenantDisplayName'] = tenant_display_name
    return result
