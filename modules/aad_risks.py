...
def _graph_get(token,url,warnings,label):
    try:
        request=urllib.request.Request(url,headers={'Authorization':f'Bearer {token}','Accept':'application/json'})
        with urllib.request.urlopen(request,timeout=15) as response:
            value=json.loads(response.read().decode());return value if isinstance(value,dict) else None
    except urllib.error.HTTPError as exc:warnings.append(f'{label}: Microsoft Graph returned HTTP {exc.code}')
...
def _graph_risky_user(token,account,warnings):
    filt=f"id eq '{account['id']}'" if account['id'] else f"userPrincipalName eq '{account['upn'].replace(chr(39),chr(39)*2)}'";select='id,userPrincipalName,userDisplayName,riskLevel,riskState,riskDetail,riskLastUpdatedDateTime'
    data=_graph_get(token,_graph_url('identityProtection/riskyUsers',{'$select':select,'$filter':filt}),warnings,'Risky user lookup');values=data.get('value',[]) if data else [];return values[0] if values and isinstance(values[0],dict) else {}
def _graph_risk_detections(token,account,warnings):
    ...
    rows.sort(key=lambda x:str(x.get('DetectedDateTime') or x.get('ActivityDateTime') or ''),reverse=True);return rows
...
def _graph_profile(token,account,warnings):
    ...
    if user_id:
        manager_data=_graph_get(token,_graph_url('users/'+urllib.parse.quote(user_id,safe='-_')+'/manager',{'$select':'id,displayName,userPrincipalName'}),warnings,'Manager lookup');manager=manager_data if isinstance(manager_data,dict) else {}
...
def query_aad_risks(req):
    accounts=_accounts(req.base);days=max(1,min(int(req.lookback_days),90));credential=DefaultAzureCredential(exclude_interactive_browser_credential=True);token=credential.get_token('https://graph.microsoft.com/.default').token;logs=LogsQueryClient(credential);details=[];warnings=[];all_events=[]
    for account in accounts:
        aw=[];failed=fraud=0;upn=account['upn'];profile,profile_uid=_graph_profile(token,account,aw);uid=profile_uid or account['id'];incident_account={'id':uid,'upn':upn};risky=_graph_risky_user(token,incident_account,aw);events=_graph_risk_detections(token,incident_account,aw)
        for event in events:all_events.append({'UserPrincipalName':upn,'UserId':uid,**event})
        registration=_graph_registration(token,uid,aw);roles=_graph_roles(token,uid,aw);risk=str(risky.get('riskLevel') or 'none').lower() if risky else 'none';risk='unknown' if risk in ('hidden','unknownfuturevalue') else risk
        if upn and (req.mfa_failure_lookup or req.mfa_fraud_lookup):
            ...
            except Exception as exc:aw.append(f'MFA telemetry query failed ({type(exc).__name__})')
        detail={'UserFailedMFACount':failed,'UserMFAFraudCount':fraud,'UserId':uid,'UserPrincipalName':upn,'UserRiskLevel':risk,'UserRiskState':risky.get('riskState') if risky else None,'UserRiskDetail':risky.get('riskDetail') if risky else None,'UserRiskLastUpdated':risky.get('riskLastUpdatedDateTime') if risky else None,'RiskEventCount':len(events),'AADRoles':roles,**profile,**registration}
        if aw:detail['EnrichmentWarnings']=sorted(set(aw))
        details.append(detail);warnings.extend(aw)
    highest=max((x['UserRiskLevel'] for x in details),key=lambda x:RANK.get(x,0),default='unknown');return {'AnalyzedEntities':len(details),'FailedMFATotalCount':sum(x['UserFailedMFACount'] for x in details),'HighestRiskLevel':highest,'MFAFraudTotalCount':sum(x['UserMFAFraudCount'] for x in details),'RiskEventCount':len(all_events),'RiskEvents':all_events,'ModuleName':'AADRisksModule','DetailedResults':details,'EnrichmentWarnings':sorted(set(warnings))}
