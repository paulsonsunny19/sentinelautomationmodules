...
def incident_context(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if missing:
        return response({"error":"missing_parameters","missing":missing,"correlationId":correlation_id}, 400)
    try:
...
def related_alerts(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if missing:
        return response({"error":"missing_parameters","missing":missing,"correlationId":correlation_id}, 400)
    try:
        request = RelatedAlertsRequest(workspace_id=body["workspaceId"],entity_value=body["entityValue"],entity_column=body["entityColumn"],lookback_hours=int(body.get("lookbackHours",24)))
