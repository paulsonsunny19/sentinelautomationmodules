...
def incident_context(req: func.HttpRequest) -> func.HttpResponse:
    correlation_id = str(uuid.uuid4()); body, error = body_json(req, correlation_id)
...
def stat_base(req: func.HttpRequest) -> func.HttpResponse:
    cid=str(uuid.uuid4()); body,error=body_json(req,cid)
    if error:return error
    missing=[x for x in ("entities","incidentArmId","workspaceId") if body.get(x) is None]
    if missing:return response({"error":"missing_parameters","missing":missing,"correlationId":cid},400)
    return response({**normalize(body["entities"],body["incidentArmId"],body["workspaceId"],body.get("tenantId"),body.get("tenantDisplayName")),"correlationId":cid})

@app.route(route="stat_scoring", methods=["POST"])
def stat_scoring(req: func.HttpRequest) -> func.HttpResponse:
    cid=str(uuid.uuid4()); body,error=body_json(req,cid)
    if error:return error
    inputs=body.get("inputs")
    if not isinstance(inputs,list):return response({"error":"missing_parameters","missing":["inputs"],"correlationId":cid},400)
    return response({**calculate(inputs),"correlationId":cid})
...
def related_alerts(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return response({"ModuleName":"RelatedAlerts","AnalyzedEntities":1,"DetailedResults":rows,"Alerts":rows,"count":len(rows),"correlationId":correlation_id})
    except ValueError as exc: return response({"error":"invalid_parameters","message":str(exc),"correlationId":correlation_id}, 400)
    except Exception:
